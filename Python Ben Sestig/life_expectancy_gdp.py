...
print("The represented years are from " + str(min(df.Year)) + " to " + str(max(df.Year)))
...
print("No, they don't - the values in de GDP chart aren't averages. It is like comparing average income to the cummulative sum of LEABY per country: Useless. The GDP should be per inhabitant.")
...
print("Which countries' bars changes the most?")
print("Zimbabwe's")
print("")

print("What years are there the biggest changes in the data?")
print("Judging the incremental steps by eye would make me think these are the years 2007, 2008 and 2009.")
print("")

print("Which country has had the least change in GDP over time?")
print("In absolute numbers, this is Zimbabwe. We would need to normalize the data to check relative changes")
print("")

print("How do countries compare to one another?")
print("They all have a growth in both GDP and LEABY and this trend seems relative constant for all 6 countries")
print("")

print("Now that you can see the both bar charts, what do you think about the relationship between GDP and life expectancy?")
print("Looking at the trends, there seems to be a positive statistical relationship between GDP and life expectancy. Judging the causality, this would also make sense as more wealth allows people to live a better life or even more impactful: be the difference between being able to buy a certain type of medicine or not.")
print("")

print("Can you think of any reasons that the data looks like this for particular countries?")
print("Zimbabwe's relative bigger change in LEABY could be due to regression to the mean.")
...
print("Which country moves the most along the X axis over the years?")
print("China's GDP has grown the most so it has moved the most along the X axis")
print("")
print("Which country moves the most along the Y axis over the years?")
print("Zimbabwe's Life expectancy has grwon the most so it has moved the most along the Y axis")
print("")
print("Is this surprising?")
print("Since the dots respond to the data and the data shows that it has grown the most, no this is not surprising. If we add my superficial view on world economics in that time period it is not surprising either. Zimbabwe's distance from the world's average on life expectancy was the biggest, so they had the most opportunity to increase life expectancy by adopting medical improvements from other countries. China and United States of America had the highest GDP from the start, if the whole world's GDP would grow by 1% this would mean a bigger absolute increas in USD for these two countries. That one of those two countries make the biggest jump on X axis is not surprising because of this.")
print("")
print("Do you think these scatter plots are easy to read? Maybe there's a way to plot that!")
print("It took a lot of mental effort to dig through the numbers, so I believe there are better ways to present comparisons over time, such as line graphs")
...
g4 = (g4.map(plt.plot, "Year", "GDP", color="brown").add_legend())
plt.show()
...
print("Which countries have the highest and lowest GDP?")
print("")
print(df.loc[df["Year"] == 2015])
print("")
print("The highest GDP belongs to the USA with a GDP of " + str(max(df.loc[df["Year"] == 2015].GDP)/(10**12)) + " trillion USD")
print("The lowest GDP belongs to Zimbabwe with a GDP of " + str("%0.1f" % (min(df.loc[df["Year"] == 2015].GDP)/(10**9))) + " billion USD")
...
print("The highest life expectancy belongs to Germany with a LEABY of " + str(max(df.loc[df["Year"] == 2015].LEABY)) + " year")
print("The lowest life expectancy belongs to Zimbabwe with a LEABY of " + str(min(df.loc[df["Year"] == 2015].LEABY)) + " year")
print("The lowest life expectancy belongs to Zimbabwe with a LEABY of " + str(min(df.LEABY)) + " year")
...
print("Why did Zimbabwe's Life Expectancy decrease after the Millennium change?")
...
#Drawing the violin plot. I want the graph to be nice and tight and easily readable, so I need to change the order to make Zimbabwe stand out
sns.set_style("whitegrid")
sns.despine(left = True, bottom = True)
fig = plt.subplots(figsize=(10, 8)) 
ax = plt.subplot()
leaby_order = ["Zimbabwe", "China", "Mexico", "United States of America", "Chile", "Germany"]
leaby_order_usa = ["Zimbabwe", "China", "Mexico", "USA", "Chile", "Germany"]
...
sns.set_context("notebook", font_scale = 1.3, rc = {"grid.linewidth": 0})
sns.despine(left = True, bottom = True)
ax.set_xticklabels(leaby_order_usa)
plt.xticks(rotation=30)
plt.title("The spread of Life Expectancy from 2000 to 2015")
...
zimbabwe = df.iloc[80:95]
usa = df.iloc[64:79]
mexico = df.iloc[48:63]
germany = df.iloc[32:47]
china = df.iloc[16:31]
chile = df.iloc[0:15]
...
plt.plot(df.iloc[0:16].Year, df.iloc[0:16].LEABY, color="#67a9cf")
plt.plot(df.iloc[16:32].Year, df.iloc[16:32].LEABY, color="#ef8a62")
plt.plot(df.iloc[32:48].Year, df.iloc[32:48].LEABY, color="#2166ac")
plt.plot(df.iloc[48:64].Year, df.iloc[48:64].LEABY, color="#fddbc7")
plt.plot(df.iloc[64:80].Year, df.iloc[64:80].LEABY, color="#d1e5f0")
plt.plot(df.iloc[80:96].Year, df.iloc[80:96].LEABY, color="#b2182b")
...
plt.subplot(1,2,2)
plt.plot(df.iloc[0:16].Year, df.iloc[0:16].GDP, color="#67a9cf")
plt.plot(df.iloc[16:32].Year, df.iloc[16:32].GDP, color="#ef8a62")
plt.plot(df.iloc[32:48].Year, df.iloc[32:48].GDP, color="#2166ac")
plt.plot(df.iloc[48:64].Year, df.iloc[48:64].GDP, color="#fddbc7")
plt.plot(df.iloc[64:80].Year, df.iloc[64:80].GDP, color="#d1e5f0")
plt.plot(df.iloc[80:96].Year, df.iloc[80:96].GDP, color="#b2182b")
plt.legend(countries, loc = 6)
plt.ylabel("GDP ($)")
...
g = sns.FacetGrid(df.iloc[range(0,96,2)], col="Year", hue="Country", col_wrap=4, size=3, palette= "RdBu", hue_order=leaby_order)
g = (g.map(plt.scatter, "GDP", "LEABY", edgecolor="w").add_legend())
g.fig.suptitle("A grid showing screenshots per year of GDP to LEABY")
...
plt.plot(df.iloc[80:96].Year, df.iloc[80:96].GDP, color="#b2182b")
sns.despine(left = True, bottom = True)
plt.title("GDP Zimbabwe from 2000 to 2015")
plt.ylabel("GDP ($)")
...
plt.plot(df.iloc[84:95].GDP, df.iloc[84:95].LEABY, marker = "o", color = "#b2182b", linestyle = ":", label = "Post-African War")
plt.plot(df.iloc[80:84].GDP, df.iloc[80:84].LEABY, marker = "d", color = "#053061", linestyle = ":", label = "African War")
plt.legend()
plt.xlabel("GDP ($)")
plt.ylabel("LEABY")

sns.despine(left = True, bottom = True)
plt.title("GDP to LEABY Zimbabwe (2000 - 2015)")

plt.savefig('Zimbabwe.png')
